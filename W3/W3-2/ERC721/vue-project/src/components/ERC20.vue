...
export default {
  ...
  methods: {
    ...
    balanceOf() {
      let address = this.checkAddress;
      this.bank.balanceOf(address).then((r) => {
        let balance =  ethers.utils.formatUnits(r, 18)
        console.log(balance);  // 返回值不是true
        this.getInfo();
      })
    },
    ...
    stake(){
      ...
      // this.erc20Token.approve(bankAddr.address,amount).send({"from":this.account}).then(function(receipt){
      //   // 当这个 receipt 对象来自于 "contract.deploy({...}).send()" 这么个调用时，它也可以是一个新合约实例
      //   console.log(receipt)
      // });

      this.erc20Token.approve(bankAddr.address,amount,{"from":this.account}).
      then(() => {
                this.getInfo();
                this.getNonce();
            })
...
<template>
  <div >
    ...
    <div >
      <input v-model="stakeAmount" placeholder="输入存储量"/>
      <button @click="stake">存款</button>
    </div>

    <div >
      <input v-model="checkAddress" placeholder="输入地址"/>
      <button @click="balanceOf">查询Vault存储的Token数量</button>
    </div>
